...
`;
    ...
    console.log(`[Gemini][Discovery] 发送分析请求 (Model: gemini-3.1-pro-preview)...`);
...
1) 前5秒内容拆解：描述前5秒具体镜头、话术、悬念设计等，并给出亮点。同时提取前5秒出现的关键内容标签（如场景、人物、道具、文案风格、视觉效果等）。
...
`;
  ...
  async analyzeNarratives(apiKey, base64Video, mimeType) {
    ...
    console.log('[Gemini][Discovery] 发送叙事洞察分析请求 (Model: gemini-3.1-pro-preview)...');
